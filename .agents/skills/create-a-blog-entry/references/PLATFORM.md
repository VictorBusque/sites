...
a visual guided experience second. Every post inlines the scaffold's runtime,
which owns the expensive, repeated behavior; the post owns its copy, its
truthful prose, and its named visual states.

## Decisions baked into the inlined runtime

| Default | Why it ships in the runtime | Platform basis |
|---|---|---|
...
| One viewport per mobile step, with final-stage reserve | A card begins bottom-docked, has a stable reading interval, and cannot escape into the next prose section. | Site regression proof; implemented in the scaffold's inlined styles and scene runtime |
...
The inlined runtime provides the card, conventional step label, progress rail,
activation, bottom sheet, and fallback. A page must not add a scroll listener,
progress control, or step observer for this scene — unless it deliberately
replaces the runtime, preserving every fallback above.
...
`scripts/check_posts.py` enforces the structural parts (manifest binding,
standalone-ness, scene honesty). The content and visual questions remain an
author review, because code cannot judge whether a story is honest or clear.
